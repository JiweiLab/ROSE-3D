function [resultfitter, lp, resultpmap] = fitXYPlane_wt(xx, yy, pmap, wmap)
    if nargin <4
        % no weight 
        wmap = ones(size(pmap));
    end
    ...
    resultfitter = @(xx,yy)(lp(1) + lp(2).*xx + lp(:,3).*yy+ lp(4).*xx.^2 + lp(5).*xx.*yy + lp(6).*yy.^2);
...
function fresult = myPlaneFunc2(xp,xdata, ydata, wdata)
    ...
    xx = xdata(:,1);
    yy = xdata(:,2);
    p = xp(1) + xp(2).*xx + xp(3).*yy + xp(4).*xx.^2 + xp(5).*xx.*yy + xp(6).*yy.^2;
